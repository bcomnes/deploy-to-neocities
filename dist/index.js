...
var require_fetch_errors = __commonJS({
  "node_modules/fetch-errors/index.js"(exports, module2) {
    async function handleResponse(response) {
      ...
      if (response.ok)
        ...
        return isJSON ? Promise.reject(new JSONHTTPError(response, data)) : Promise.reject(new TextHTTPError(response, data));
      }
    }
...
var require_async_folder_walker = __commonJS({
  "node_modules/async-folder-walker/index.js"(exports, module2) {
    "use strict";
    var fs = require("fs");
    var path2 = require("path");
    var ignore = require_ignore();
    var { readdir, lstat } = fs.promises;
    var pathFilter = (filepath) => true;
    var statFilter = (st) => true;
    var shaper = ({ root, filepath, stat, relname: relname2, basename }) => filepath;
      ...
      while (roots.length) {
        ...
        while (pending.length) {
          ...
          for (const file of files) {
            const next = path2.join(current, file);
            if (opts.pathFilter(next))
              pending.unshift(next);
          }
          if (current === root || !opts.statFilter(st))
            continue;
          else
            yield opts.shaper(fwShape(root, current, st));
        }
    ...
    function relname(root, name) {
      return root === name ? path2.basename(name) : path2.relative(root, name);
    }
    function fwShape(root, name, st) {
      return {
        ...
        basename: path2.basename(name)
      };
    }
    function depthLimiter(filePath, relativeTo, maxDepth) {
      if (maxDepth === Infinity)
        return false;
      const rootDepth = relativeTo.split(path2.sep).length;
      const fileDepth = filePath.split(path2.sep).length;
...
var require_helpers = __commonJS({
  "node_modules/pony-cause/lib/helpers.js"(exports, module2) {
    "use strict";
...
var require_folder_diff = __commonJS({
  "node_modules/async-neocities/lib/folder-diff.js"(exports, module2) {
    ...
    async function neocitiesLocalDiff(neocitiesFiles, localListing, opts = {}) {
      ...
      ncFiles.forEach((ncFile) => {
        if (opts.protectedFileFilter(ncFile))
          protectedFiles.add(ncFile);
      });
      ...
      for (const p of maybeUpdate) {
        ...
        if (local.stat.size !== remote.size) {
          filesToAdd.add(p);
          continue;
        }
        const localSha1 = await sha1FromPath(local.filepath);
        if (localSha1 !== remote.sha1_hash) {
          filesToAdd.add(p);
          continue;
        }
        skipped.add(p);
...
var require_package = __commonJS({
  "node_modules/async-neocities/package.json"(exports, module2) {
    module2.exports = {
...
var require_timer = __commonJS({
  "node_modules/async-neocities/lib/timer.js"(exports, module2) {
...
var require_stream_meter = __commonJS({
  "node_modules/async-neocities/lib/stream-meter.js"(exports, module2) {
    var { Writable, Transform } = require_streamx();
    var pump = require_pump();
...
var require_stats_handler = __commonJS({
  "node_modules/async-neocities/lib/stats-handler.js"(exports, module2) {
...
var require_lodash = __commonJS({
  "node_modules/lodash.chunk/index.js"(exports, module2) {
    var INFINITY = 1 / 0;
...
// node_modules/async-neocities/lib/create-form.js
var require_create_form = __commonJS({
  "node_modules/async-neocities/lib/create-form.js"(exports) {
    var FormData = require_form_data();
    var batch = require_lodash();
...
var require_async_neocities = __commonJS({
  "node_modules/async-neocities/index.js"(exports, module2) {
    var { handleResponse } = require_fetch_errors();
    var { createReadStream } = require("fs");
    var afw = require_async_folder_walker();
    var assert2 = require_nanoassert();
    var fetch = require_lib3();
    var { URL: URL3 } = require("url");
    var qs = require("querystring");
    var os = require("os");
    var { ErrorWithCause } = require_pony_cause();
    var { neocitiesLocalDiff } = require_folder_diff();
    var pkg = require_package();
    var SimpleTimer = require_timer();
    var { getStreamsLength, getStreamLength, meterStream, captureStreamLength } = require_stream_meter();
    var statsHandler = require_stats_handler();
    var { createForm, createForms } = require_create_form();
    var defaultURL = "https://neocities.org";
    var START = "start";
    var PROGRESS = "progress";
    var STOP = "stop";
    var SKIP = "skip";
    var ERROR = "error";
    var INSPECTING = "inspecting";
    var DIFFING = "diffing";
    var APPLYING = "applying";
    var NeocitiesAPIClient = class {
      /**
      ...
      static getKey(sitename, password, opts) {
        assert2(sitename, "must pass sitename as first arg");
        assert2(typeof sitename === "string", "user arg must be a string");
        assert2(password, "must pass a password as the second arg");
        assert2(typeof password, "password arg must be a string");
        opts = Object.assign({
          url: defaultURL
        }, opts);
        const baseURL = opts.url;
        delete opts.url;
        const url = new URL3("/api/key", baseURL);
        url.username = sitename;
        url.password = password;
        return fetch(url, opts);
      }
      static statsHandler(...args) {
        return statsHandler(...args);
      }
      /**
      ...
      constructor(apiKey, opts) {
        assert2(apiKey, "must pass apiKey as first argument");
        assert2(typeof apiKey === "string", "apiKey must be a string");
        opts = Object.assign({
          url: defaultURL
        });
        this.url = opts.url;
      ...
      get(endpoint, quieries, opts) {
        assert2(endpoint, "must pass endpoint as first argument");
        opts = Object.assign({
          method: "GET"
        }, opts);
        opts.headers = Object.assign({}, this.defaultHeaders, opts.headers);
        let path2 = `/api/${endpoint}`;
        if (quieries)
          path2 += `?${qs.stringify(quieries)}`;
        const url = new URL3(path2, this.url);
        return fetch(url, opts);
      }
      /**
      ...
      async post(endpoint, formEntries, opts) {
        assert2(endpoint, "must pass endpoint as first argument");
        assert2(formEntries, "must pass formEntries as second argument");
        opts = Object.assign({
          method: "POST",
          statsCb: () => {
          }
        ...
        const url = new URL3(`/api/${endpoint}`, this.url);
        return fetch(url, opts);
      }
      /**
      ...
      upload(files, opts = {}) {
        ...
        const formEntries = files.map(({ name, path: path2 }) => {
          const streamCtor = (next) => next(createReadStream(path2));
          streamCtor.path = path2;
          return {
            name,
            value: streamCtor
          };
      ...
      delete(filenames, opts = {}) {
        assert2(filenames, "filenames is a required first argument");
        assert2(Array.isArray(filenames), "filenames argument must be an array of file paths in your website");
        opts = {
          statsCb: () => {
          },
          ...opts
      ...
      async deploy(directory, opts) {
        opts = {
          cleanup: false,
          // delete remote orphaned files
          statsCb: () => {
          },
          protectedFileFilter: (path2) => false,
          // no protected files by default
          ...opts
        };
        const statsCb = opts.statsCb;
    ...
    module2.exports = NeocitiesAPIClient;
  }
});
...
// node_modules/webassert/cjs/index.js
var require_cjs = __commonJS({
  "node_modules/webassert/cjs/index.js"(exports) {
    "use strict";
    var AssertionError = class extends Error {
    };
    AssertionError.prototype.name = "AssertionError";
    ...
    Object.defineProperty(exports, "__esModule", { value: true }).default = assert2;
  }
});
...
var require_cjs2 = __commonJS({
  "node_modules/minimatch/dist/cjs/index.js"(exports) {
    ...
    var path2 = isWindows ? { sep: "\\" } : { sep: "/" };
    exports.sep = path2.sep;
    ...
    var Minimatch = class {
      ...
      make() {
        ...
        this.globParts = this.options.noglobstar ? rawGlobParts : rawGlobParts.map((parts) => parts.reduce((set2, part) => {
          if (part !== "**" || set2[set2.length - 1] !== "**") {
            set2.push(part);
          }
          return set2;
        }, []));
      ...
      makeRe() {
        ...
        let re = set.map((pattern) => {
          const pp = pattern.map((p) => typeof p === "string" ? regExpEscape(p) : p === exports.GLOBSTAR ? exports.GLOBSTAR : p._src);
          pp.forEach((p, i) => {
            const next = pp[i + 1];
            const prev = pp[i - 1];
...
var Neocities = require_async_neocities();
var path = require("path");
var ms = require_ms();
var assert = require_cjs().default;
var fsp = require("fs").promises;
var minimatch = require_cjs2();
var { stackWithCauses } = require_pony_cause();
var cleanup;
async function doDeploy() {
  ...
  if (protectedFilesGlob)
    deployOpts.protectedFileFilter = minimatch.filter(protectedFilesGlob);
